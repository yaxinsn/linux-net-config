...
int __send_to_server(void* msg,int len)
{
	...
	{
		ret = send(upload_ctx.main_fd,msg,len);
		if(ret != 0)
		{
	...
	{
		ret = send(upload_ctx.bak_fd,msg,len);
		if(ret != 0)
			upload_ctx.warn_event = 2;
	}
	
}
static int __send_talking(void* msg,int len)
{
...
static void upload_open_socket()
{
	upload_close_socket();
  	...
  	upload_ctx.back_fd = connect_tcp_socket(&g_config.tape.spareip,g_config.tape.spareport);
...
static void upload_close_socket()
{
	...
	if(upload_ctx.back_fd >0)
		close(upload_ctx.back_fd);	
	upload_ctx.back_fd = 0;	
}
...
int uploader_push_msg(void* msg,int len)
{
	...
	return msg_engine_push_entry(upload_ctx->me,msg,len);
